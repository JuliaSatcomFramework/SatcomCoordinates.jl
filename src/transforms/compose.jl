...
function _compose(t1::RawComposedTransform, t2::RawComposedTransform)
    if t1.t2 isa Identity && t2.t1 isa Identity
        return RawComposedTransform(t1.t1, t2.t2)
    else
        throw(ArgumentError("The `_compose(t1::RawComposedTransform, t2::RawComposedTransform)` method only works if both `t1.t2` and `t2.t1` are the Identity transform."))
    end
end